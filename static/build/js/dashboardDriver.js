...
var steps = [
    { popover: { title: 'Dashboard', description: 'Task dashboard section' } },
...
if ($('.oh-card-dashboard').length) {
    steps.push({ element: '#tileContainer .oh-card-dashboard:nth-child(1)', popover: { title: 'Dashboard Tiles', description: 'Task Dashboard Tiles' } });
}
setTimeout(() => {
    ...
    if ($('[data-id="dashboardNav"]').length) {
        steps.push({ element: '[data-id="dashboardNav"]', popover: { title: 'App', description: 'Task Hr Apps. eg Dashboard' } });
    }
}, 1000);
driverObj = driver(
// Driver options
